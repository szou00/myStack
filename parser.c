...
void parse_file ( char * filename,
  ...
  while ( fgets(line, sizeof(line), f) != NULL ) {
    ...
    if (strncmp(line, "push", strlen(line)) == 0) {
      ...
      matrix_mult(peek(csystems), tmp); //multiply top of stack with tmp, tmp becomes the product
      copy_matrix(tmp, peek(csystems)); //the product becomes the top of the stack
    }//end translate
